...
export const SkeletonLoader: React.FC<SkeletonLoaderProps> = ({
  ...
  if (type === 'avatar') {
    return (
      <div className={`animate-pulse bg-slate-200 dark:bg-slate-800 rounded-full ${className || 'w-10 h-10'}`} />
    );
  }
    ...
    return (
      <div className={`space-y-3 ${className}`}>
        {items.map((_, idx) => (
          <div
            key={idx}
            className="animate-pulse p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-3 shadow-xs"
          >
  ...
  if (type === 'table') {
    return (
      <div className={`animate-pulse space-y-3 ${className}`}>
        ...
        {items.map((_, idx) => (
          <div key={idx} className="h-12 bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 rounded-lg w-full flex items-center px-4 gap-4">
            <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-1/6" />
            <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-1/3" />
            <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-1/4" />
            <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-1/12 ml-auto" />
          </div>
        ))}
      </div>
    );
  ...
  if (type === 'detail') {
    return (
      <div className={`animate-pulse space-y-6 ${className}`}>
        {/* Header Skeleton */}
        <div className="space-y-3 p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs">
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ${className}`}>
      {items.map((_, idx) => (
        <div
          key={idx}
          className="animate-pulse bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-4 shadow-xs flex flex-col justify-between"
        >
