...
export const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose }) => {
 ...
 return (
 <div
 className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-start justify-center pt-16 px-4 animate-in fade-in duration-200"
 onClick={onClose}
 >
 <div
 className="w-full max-w-2xl bg-white border border-slate-200 shadow-2xl overflow-hidden flex flex-col max-h-[80vh] text-slate-900 transition-all"
 ...
 className="p-1 rounded-md text-slate-400 hover:text-slate-600 :text-slate-200 hover:bg-slate-200/50 :bg-slate-800 mr-2"
 >
 <X className="w-4 h-4" />
 </button>
 ) : null}
 <kbd className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-mono font-bold text-slate-500 bg-slate-200/60 border border-slate-300/60 rounded-md">
 ESC
 </kbd>
 </div>

 {/* Results List */}
 <div ref={resultsRef} className="flex-1 overflow-y-auto p-2 space-y-1 divide-y divide-slate-100 ">
 {combinedItems.length === 0 ? (
 <div className="p-8 text-center text-slate-500 space-y-2">
 <Sparkles className="w-8 h-8 mx-auto text-slate-400 animate-pulse" />
 <p className="text-sm font-semibold text-slate-800 ">No matching routes or entities</p>
 ...
 : 'text-slate-700 hover:bg-slate-100 :bg-slate-800/70'
 }`}
 >
 ...
 {/* Footer Shortcut Bar */}
 <div className="px-4 py-2.5 bg-slate-100/80 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-500 font-mono">
 <div className="flex items-center gap-4">
 <span className="flex items-center gap-1">
 <kbd className="px-1.5 py-0.5 bg-white border border-slate-300 rounded shadow-xs text-[10px]">
 ↑
 </kbd>
 <kbd className="px-1.5 py-0.5 bg-white border border-slate-300 rounded shadow-xs text-[10px]">
 ↓
