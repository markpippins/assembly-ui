...
import { DataService, Candidate, DEFAULT_PAGE_SIZE } from '../../services/data.service';
...
export class CandidatesViewComponent implements OnInit {
  ...
  sortedItems = computed(() => sortItems<Candidate>(this.items(), this.sortField(), this.sortDir()));
  ...
  private load() {
    ...
    this.error.set(null);
    this.dataService.getCandidates(this.currentPage(), this.pageSize).subscribe({
      next: res => {
