...
import { DataService, Requirement, DEFAULT_PAGE_SIZE } from '../../services/data.service';
...
export class RequirementsViewComponent implements OnInit {
  ...
  sortedItems = computed(() => sortItems<Requirement>(this.items(), this.sortField(), this.sortDir()));
  ...
  private load() {
    ...
    this.error.set(null);
    this.dataService.getRequirements(this.currentPage(), this.pageSize).subscribe({
      next: res => {
