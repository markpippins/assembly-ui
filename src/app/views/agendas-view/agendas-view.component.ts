...
import { DataService, Agenda, DEFAULT_PAGE_SIZE } from '../../services/data.service';
...
export class AgendasViewComponent implements OnInit {
  ...
  sortedItems = computed(() => sortItems<Agenda>(this.items(), this.sortField(), this.sortDir()));
  ...
  private load() {
    ...
    this.error.set(null);
    this.dataService.getAgendas(this.currentPage(), this.pageSize).subscribe({
      next: res => {
