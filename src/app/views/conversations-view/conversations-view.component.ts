...
import { DataService, ConversationSnapshot, DEFAULT_PAGE_SIZE } from '../../services/data.service';
...
export class ConversationsViewComponent implements OnInit {
  ...
  sortedItems = computed(() => sortItems<ConversationSnapshot>(this.items(), this.sortField(), this.sortDir()));
  ...
  private load() {
    ...
    this.error.set(null);
    this.dataService.getConversations(this.currentPage(), this.pageSize).subscribe({
      next: res => {
