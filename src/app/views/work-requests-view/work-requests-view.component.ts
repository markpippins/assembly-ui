...
import { DataService, WorkRequest, DEFAULT_PAGE_SIZE } from '../../services/data.service';
...
export class WorkRequestsViewComponent implements OnInit {
  ...
  sortedItems = computed(() => sortItems<WorkRequest>(this.items(), this.sortField(), this.sortDir()));
  ...
  private load() {
    ...
    this.error.set(null);
    this.dataService.getWorkRequests(this.currentPage(), this.pageSize).subscribe({
      next: res => {
