...
import { DataService, IntentRecord, DEFAULT_PAGE_SIZE } from '../../services/data.service';
...
export class IntentsViewComponent implements OnInit {
  ...
  sortedItems = computed(() => sortItems<IntentRecord>(this.items(), this.sortField(), this.sortDir()));
  ...
  private load() {
    ...
    this.error.set(null);
    this.dataService.getIntents(this.currentPage(), this.pageSize).subscribe({
      next: res => {
