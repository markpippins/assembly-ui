      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-1.5 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 py-1.5 text-sm font-sans animate-in fade-in zoom-in-95 duration-100">
          ...
          <button
            ...
            className="w-full text-left px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 flex items-center gap-2 cursor-pointer transition-colors"
          >
          ...
          <button
            ...
            className={`w-full text-left px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 cursor-pointer transition-colors ${
              isFlagged ? 'text-amber-600 dark:text-amber-400 font-medium' : 'text-slate-700 dark:text-slate-200'
            }`}
          >
...
export const HarvestDetailView: React.FC = () => {
  ...
  const discourseUnits: any[] = docklang?.discourseUnits || [];
    ...
    <div className="max-w-6xl mx-auto py-6 px-4 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-500 dark:text-slate-400">
        <div className="flex items-center gap-2">
          ...
          <span>/</span>
          <span className="text-slate-800 dark:text-slate-200 font-mono font-medium truncate max-w-xs">{title}</span>
        </div>
      ...
      {/* Header Info Card */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-sm transition-all">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {harvest.model && (
                <Tooltip content="AI Model: Generative intelligence model used to harvest and classify discourse blocks.">
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 cursor-help">
              ...
              <Tooltip content="Abstraction Level: Level 3 signifies deep discourse segmentation with aggregated turns and typed blocks.">
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 cursor-help">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white font-poppins tracking-tight">
              {title}
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-950/60 p-4 rounded-xl border border-slate-200/80 dark:border-slate-800/80 text-sm font-mono">
          <div className="space-y-1">
            ...
            <p className="text-slate-800 dark:text-slate-200 truncate font-mono text-[11px]" title={provenance.source || harvest.sourcePath || ''}>
          ...
          <div className="space-y-1">
            ...
            <p className="text-slate-800 dark:text-slate-200 font-mono text-[11px] truncate">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm font-semibold text-slate-700 dark:text-slate-300 font-mono">
            ...
            <Tooltip content="Totals: Aggregate discourse units (turns) and total content blocks.">
              <span className="cursor-help underline decoration-dotted underline-offset-2">
                {stats.totalUnits || discourseUnits.length} Discourse Units • {stats.totalBlocks || 0} Total Blocks
              </span>
            </Tooltip>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            <Tooltip content="Discourse Units: Aggregated conversational turns in this harvest document.">
              <div className="bg-indigo-50/50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/40 rounded-xl p-3 text-center cursor-help transition-transform hover:-translate-y-0.5">
                <div className="text-lg font-bold text-indigo-600 dark:text-indigo-400 font-mono">
                  {stats.totalUnits || discourseUnits.length}
                </div>
            ...
            <Tooltip content="Paragraph Blocks: Count of standard narrative text blocks extracted across turns.">
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-xl p-3 text-center cursor-help transition-transform hover:-translate-y-0.5">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 rounded-2xl shadow-xs">
        {/* Toggle Switch */}
            ...
            filteredUnits.map((unit, idx) => {
              ...
              return (
                <div
                  ...
                  } rounded-2xl shadow-sm transition-all overflow-hidden`}
                >
                  {/* Turn Header Card */}
                  <div
                    className={`p-4 sm:p-5 flex flex-wrap items-center justify-between gap-3 border-b ${
                      isTurnFlagged
                        ...
                        : isAssistant
                        ? 'bg-slate-50/70 dark:bg-slate-950/40 border-slate-100 dark:border-slate-800/80'
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {isTurnFlagged && (
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-800">
                  ...
                  {!isCollapsed && (
                    <div className="p-6 sm:p-8 space-y-4 font-sans leading-relaxed text-slate-800 dark:text-slate-200 text-sm">
                      {Array.isArray(unit.blocks) && unit.blocks.length > 0 ? (
                        unit.blocks.map((block: any, bIdx: number) => {
                          ...
                          const blockWrapperClass = `relative group/block rounded-xl border p-3.5 transition-all ${
                            isBlockFlagged
                              ? 'border-amber-300 dark:border-amber-700/80 bg-amber-50/30 dark:bg-amber-950/20 shadow-xs'
                              : 'border-slate-100 dark:border-slate-800/80 hover:border-slate-200 dark:hover:border-slate-700 bg-white/50 dark:bg-slate-900/50'
                          }`;

                          const renderBlockHeader = () => (
                            <div className="flex items-center justify-between gap-2 pb-2 mb-2 border-b border-slate-100 dark:border-slate-800/60">
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-mono font-semibold uppercase tracking-wider text-slate-400">
                                  #{bIdx + 1} {type}
                                </span>
                                {isBlockFlagged && (
                                  <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-amber-600 dark:text-amber-400 bg-amber-100/80 dark:bg-amber-950/80 px-2 py-0.5 rounded-md border border-amber-300 dark:border-amber-800">
                                    <Flag className="w-3 h-3 fill-amber-500 text-amber-500" />
                                    Flagged for Review
                                  </span>
                                )}
                          ...
                          if (type === 'quote') {
                            return (
                              ...
                              >
                                {renderBlockHeader()}
                                <div className="bg-indigo-50/50 dark:bg-indigo-950/30 border-l-4 border-indigo-500 dark:border-indigo-400 rounded-r-xl p-3 text-slate-800 dark:text-slate-200 font-serif text-sm leading-relaxed space-y-1">
                                  <Quote className="w-4 h-4 text-indigo-400 dark:text-indigo-500 mb-1" />
                          ...
                          if (type === 'list') {
                            ...
                            return (
                              ...
                              >
                                {renderBlockHeader()}
                                <ul className="list-disc pl-5 space-y-1.5 text-slate-800 dark:text-slate-200 text-sm">
                          ...
                          if (type === 'separator') {
                            return (
                              ...
                              >
                                {renderBlockHeader()}
                                <div className="my-2 border-t border-slate-200 dark:border-slate-800 flex items-center justify-center">
                                  <span className="bg-white dark:bg-slate-900 px-3 -mt-2 text-[10px] font-mono text-slate-400 uppercase tracking-widest">
                                    •••
                                  </span>
                                </div>
                              </div>
