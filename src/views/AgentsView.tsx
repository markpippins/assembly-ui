    ...
    <div className="max-w-6xl mx-auto py-6 px-4 space-y-6">
              ...
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-700/80 text-slate-500 dark:text-slate-400 font-mono uppercase text-[10px]">
                  <th
                    ...
                    className="py-3 px-4 font-semibold cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400 select-none"
                  >
                    Title {sortField === 'title' && <span className="text-[10px]">{sortDir === 'asc' ? '▲' : '▼'}</span>}
                  </th>
                  <th
                    onClick={() => onToggleSort('role')}
                    className="w-28 py-3 px-4 font-semibold cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400 select-none"
                  >
                    Role {sortField === 'role' && <span className="text-[10px]">{sortDir === 'asc' ? '▲' : '▼'}</span>}
                  </th>
                  <th
                    onClick={() => onToggleSort('createdAt')}
                    className="w-36 py-3 px-4 font-semibold cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400 select-none"
                  >
                  ...
                  <tr key={rec.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex flex-col">
                        <Link
                          ...
                          className="font-bold text-slate-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-300 transition-colors"
                        >
                          {rec.title || 'Untitled'}
                        </Link>
                        <Link
                          to={`/agent-records/${rec.id}`}
                          className="text-xs text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-300 transition-colors"
                        >
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 hover:border-amber-300 dark:bg-amber-500/10 dark:border-amber-500/30 dark:text-amber-300 dark:hover:bg-amber-500/20 transition-all cursor-pointer"
                      >
      ...
      {raiseTarget && (
        <div
          ...
          aria-modal="true"
        >
          <div className="absolute inset-0 bg-gray-900/50 dark:bg-black/60 backdrop-blur-sm" onClick={closeRaise} />
          <div className="relative w-full sm:w-auto sm:max-w-lg sm:rounded-lg app-panel p-4 shadow-xl max-h-[90vh] sm:max-h-none overflow-y-auto">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
              ...
              <span className="font-medium text-gray-700 dark:text-gray-300">Issues</span> linked to{' '}
            ...
            <form onSubmit={submitRaise} className="space-y-3">
              <div>
                <label htmlFor="rq-title" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Title
                </label>
                <input
                  id="rq-title"
                  ...
                  className="w-full rounded border border-gray-200 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 px-2 py-1.5 text-sm focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
                  placeholder="Short summary of the question"
                  required
                />
              </div>
              <div>
                <label htmlFor="rq-body" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Body
                </label>
                <textarea
                  id="rq-body"
                  ...
                  className="w-full resize-none rounded border border-gray-200 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 px-2 py-1.5 text-sm focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
              ...
              <div className="flex items-center justify-end gap-2 mt-4">
                <button
                  type="button"
                  onClick={closeRaise}
                  className="px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
