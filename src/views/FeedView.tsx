...
export const FeedView: React.FC = () => {
 ...
 <Avatar name={currentUser?.name || 'Guest'} avatar={currentUser?.avatar || 'G'} size="md" />
 ...
 rows={3}
 className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 resize-none transition-all"
 />
 </div>
 </div>
 <div className="flex justify-between items-center pt-2 border-t border-slate-100 ">
 <span className="text-[11px] text-slate-500 ">Posts are visible to all workspace contributors</span>
 <button
 type="submit"
 ...
 <Rss className="w-8 h-8 mx-auto text-slate-400 " />
 <p className="text-sm font-semibold text-slate-900 ">No activity posts yet</p>
 <p className="text-sm text-slate-500 ">Be the first to share an update using the form above.</p>
 </div>
 ) : (
 posts.map((post) => (
 <div
 ...
 post.forum ? 'cursor-pointer hover:border-indigo-300 :border-indigo-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-500 focus-visible:outline-offset-2' : ''
 }`}
 ...
 className="p-1.5 text-slate-400 hover:text-rose-600 :text-rose-400 hover:bg-slate-100 :bg-slate-700/50 rounded-lg transition-colors"
 title="Delete post"
 >
 ...
 <div>
 <h2 className="text-base font-bold text-slate-900 font-poppins">{post.title}</h2>
 ...
 onClick={(e) => e.stopPropagation()}
 className="inline-flex items-center gap-1.5 hover:text-indigo-600 :text-indigo-400 transition-colors"
 >
