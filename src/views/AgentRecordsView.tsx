          ...
          <div key={rec.id} className="app-panel p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[10px] uppercase px-2 py-0.5 rounded bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                  Type: {rec.recordType || 'LOG'}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <TTSButton text={`${rec.title || ''}. ${rec.content || ''}`} label="Read Record" />
                <Link to={`/agent-records/${rec.id}`} className="font-mono text-[11px] text-indigo-600 dark:text-indigo-400 hover:underline">{rec.id}</Link>
              </div>
            </div>

            <h2 className="text-base font-bold text-slate-900 dark:text-white font-poppins">{rec.title}</h2>
            ...
            {rec.tags && rec.tags.length > 0 && (
              <div className="flex items-center gap-1.5 pt-1">
                {rec.tags.map((t) => (
                  <span key={t} className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                    #{t}
                  </span>
                ))}
              </div>
