  ...
  return (
    <div className="max-w-3xl mx-auto py-6 px-4 space-y-6">
      <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
        ...
        <span>/</span>
        <span className="text-slate-900 dark:text-white font-mono">{user.name}</span>
      </div>
      ...
      <div className="app-panel p-4 space-y-6">
        <div className="flex items-center gap-4 border-b border-slate-200 dark:border-slate-700/60 pb-5">
          ...
          <div>
            ...
            <p className="text-sm text-indigo-600 dark:text-indigo-400 font-mono">Contributor ID: <Link to={`/profile/${user.id}`} className="text-indigo-600 dark:text-indigo-400 hover:underline">{user.id}</Link></p>
          </div>
        </div>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Posts ({posts.length})</h2>
        {posts.length === 0 ? (
          <div className="app-panel p-4 text-center text-sm text-gray-400 dark:text-gray-500">
            <p className="font-medium text-gray-600 dark:text-gray-300">No posts</p>
            <p className="mt-1">This user has not published any posts yet.</p>
          </div>
        ) : (
          posts.map((post) => (
            <div key={post.id} className="app-panel p-4 space-y-2">
              {post.title && <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100">{post.title}</h3>}
              <MarkdownRenderer content={post.content} />
              <div className="text-[11px] text-gray-400 dark:text-gray-500">
                {formatDateTime(post.createdAt)}
                {post.forum && (
                  <Link to={`/forums/${post.forum.slug}`} className="ml-2 text-primary-600 dark:text-primary-400 hover:underline">
                    in {post.forum.name}
                  </Link>
                )}
              </div>
