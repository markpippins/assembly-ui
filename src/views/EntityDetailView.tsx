...
function conversationMarkdown(blocks: any[]): string {
  ...
  for (const block of blocks) {
    ...
    const content = (block['contentMd'] as string | undefined)
                 || (block['content_md'] as string | undefined)
                 || '';
...
export const EntityDetailView: React.FC = () => {
  ...
  const [conversationBlocks, setConversationBlocks] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    if (type === 'Conversation' && id) {
      const blocks = dataService.getConversationBlocks(id);
      if (blocks.length > 0) setConversationBlocks(blocks);
      api.fetchConversationBlocks(id).then((fetched) => {
        if (fetched && fetched.length > 0) setConversationBlocks(fetched);
      }).catch(() => {});
    }
    if (type === 'Agenda' && id) {
  ...
  const conversationMd = entityType === 'Conversation' ? conversationMarkdown(conversationBlocks) : '';
  ...
  return (
    <div className="max-w-6xl mx-auto py-6 px-4 space-y-6">
      <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
        ...
        <span>/</span>
        <Link to={location.pathname} className="text-slate-900 dark:text-white font-mono hover:text-indigo-600 dark:hover:text-indigo-400 hover:underline">{id}</Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div className="app-panel overflow-hidden">
                ...
                <div className="flex items-center gap-2">
                  {entityData.docklang && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded transition-colors bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-700"
              ...
              <div className="px-4 py-3 max-h-[70vh] overflow-y-auto">
                {!showDockLang ? (
                  ...
                  <pre className="text-xs font-mono text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-900 rounded p-3 overflow-x-auto whitespace-pre-wrap break-words">
          ...
          {/* Conversation Blocks (continuous markdown) */}
          {entityType === 'Conversation' && (
            <div className="app-panel overflow-hidden">
              <div className="px-4 py-2.5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Conversation</h2>
                <span className="text-xs text-slate-400 dark:text-slate-500">
                  {conversationBlocks.length > 0 ? `${conversationBlocks.length} blocks` : 'Loading…'}
                </span>
              </div>
              <div className="px-4 py-3 max-h-[70vh] overflow-y-auto">
                {conversationMd ? (
                  <MarkdownRenderer content={conversationMd} />
                ) : (
                  <p className="text-sm text-slate-400 dark:text-slate-500">No conversation blocks available.</p>
                )}
              </div>
            </div>
          )}
          ...
          {entityType === 'Agenda' && agendaItems.length > 0 && (
            <div className="app-panel overflow-hidden">
              <div className="px-4 py-2.5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 flex items-center justify-between">
                <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Agenda Items</h2>
                <span className="text-xs text-slate-400 dark:text-slate-500">{agendaItems.length} items</span>
              </div>
                ...
                {agendaItems.map((item, idx) => {
                  ...
                  return (
                    <div key={String(obj['id'] || obj['sourceId'] || obj['title'] || idx)} className="px-4 py-3">
                      <div className="text-sm font-medium text-slate-900 dark:text-slate-100">{getItemTitle(obj)}</div>
                      <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        {sourceType && (
                          itemRoute ? (
                            <Link to={itemRoute} className="text-indigo-600 dark:text-indigo-400 hover:underline font-medium">
                      ...
                      {getItemBody(obj) && (
                        <div className="text-sm text-slate-600 dark:text-slate-300 mt-2 line-clamp-3">{getItemBody(obj)}</div>
                      )}
                    </div>
                  );
                })}
          ...
          {metadataEntries.length > 0 && (
            <div className="app-panel overflow-hidden">
              <div className="px-4 py-2.5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 flex items-center justify-between">
                ...
                <Link to={location.pathname} className="font-mono text-[11px] text-indigo-600 dark:text-indigo-400 hover:underline">ID: {entityData.id}</Link>
              </div>
              <dl className="divide-y divide-slate-100 dark:divide-slate-800">
                {metadataEntries.map((entry) => (
                  <div key={entry.rawKey} className="px-4 py-2.5 grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <dt className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{entry.key}</dt>
                    <dd className="text-sm text-slate-900 dark:text-slate-100 sm:col-span-2 whitespace-pre-wrap break-words">
                      {entry.route ? (
                        <Link to={entry.route} className="inline-flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400 hover:underline font-semibold">
